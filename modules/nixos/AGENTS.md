...
- Bridge user programs to `flake.homeModules.<name>` for `config.custom.homeManager.username` when native Home Manager support is available.
...
- Same-flake package consumer: import from `flake.packages.<system>` rather than rebuilding locally unless host-specific configuration requires it
- NixOS-to-Home-Manager bridge: import `flake.homeModules.<name>` into the configured Home Manager user
