...
Skills use Pi tool names. When you encounter these in a skill, use your platform equivalent:
...
| `todo tool` (task tracking) | `write_todos` |
...
|-------------------|----------------------|
| `Agent tool (implementer)` | `@generalist` with the filled `implementer-prompt.md` template |
| `Agent tool (spec-reviewer)` | `@generalist` with the filled `spec-reviewer-prompt.md` template |
| `Agent tool (code-reviewer)` | `@code-reviewer` (bundled agent) or `@generalist` with the filled review prompt |
| `Agent tool (code-quality-reviewer)` | `@generalist` with the filled `code-quality-reviewer-prompt.md` template |
...
These tools are available in Gemini CLI but have no Pi equivalent:
